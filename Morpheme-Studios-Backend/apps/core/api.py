...
def login(request):
    ...
    if user is None or not user.is_active:
        audit.record(
            audit.AuditLog.Action.LOGIN_FAILED,
            target_type="User",
            target_id=serializer.validated_data["email"],
        )
        return Response(
            {
                "error": {
    ...
    if getattr(user, "mfa_enabled", False):
        audit.record(
            audit.AuditLog.Action.LOGIN, target=user, changes={"mfa": "required"}
        )
        return Response(
            {
                "mfa_required": True,
...
def refresh(request):
    ...
    if not token:
        return Response(
            {"error": {"code": "no_refresh", "message": "Missing refresh token."}},
            status=status.HTTP_401_UNAUTHORIZED,
        )
    try:
    ...
    if not user or not user.is_active:
        return Response(
            {"error": {"code": "invalid_refresh", "message": "User unavailable."}},
            status=status.HTTP_401_UNAUTHORIZED,
        )
...
def password_reset_request(request):
    ...
    if user is not None:
        ...
        audit.record(
            ...
            target=user,
            changes={"password_reset": "requested"},
        )
...
def password_reset_confirm(request):
    ...
    try:
        ...
        return Response(
            {"error": {"code": "weak_password", "message": " ".join(exc.messages)}},
            status=status.HTTP_400_BAD_REQUEST,
        )
    user.set_password(data["new_password"])
    user.save(update_fields=["password"])
    audit.record(
        audit.AuditLog.Action.UPDATE, target=user, changes={"password": "reset"}
    )
...
def change_password(request):
    ...
    user.save(update_fields=["password"])
    audit.record(
        audit.AuditLog.Action.UPDATE, target=user, changes={"password": "changed"}
    )
